...
The Web Data API does not currently provide the interval import/export totals needed to know how much of a capped benefit remains. During 4Free import hours the integration therefore reports the gross import price as the safe state. For Happy Hour and 4Free export, you can optionally select your own `kWh`, `energy`, `total_increasing` export counter in the integration options. It snapshots that counter at 5:30pm and subtracts it through the 9:30pm window, including across a restart when the saved baseline is available. Without that validated counter or a saved same-day baseline, the integration reports the guaranteed lower tier. The `rate_min_*`, `rate_max_*`, `rate_is_exact`, `cap_status`, and `uncertainty_reason` attributes expose the full range and why it is uncertain. Forecasts remain conservative because future counter usage is unknown.
